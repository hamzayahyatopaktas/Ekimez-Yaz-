...
const CACHE_VERSION = '2026-05-18-v5';
  ...
  event.respondWith((async () => {
    ...
    try {
      ...
      if (res && res.status === 200 && res.type !== 'opaque') {
        // Klonu res döndürülmeden ÖNCE al — aksi halde body tüketilmiş olabilir, clone() patlar
        const resCopy = res.clone();
        caches.open(CACHE_NAME).then(c => c.put(req, resCopy)).catch(() => {});
      }
      return res;
    } catch (e) {
      if (isNavigate) {
